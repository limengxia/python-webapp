...
需要编写一个Web框架。从使用者的角度来说，aiohttp相对比较底层.
编写一个URL的处理函数需要这么几步：
第一步，编写一个用@asyncio.coroutine装饰的函数：
第二步，传入的参数需要自己从request中获取：
最后，需要自己构造Response对象：
'''
#要把一个函数映射为一个URL处理函数，我们先定义@get()：一个函数通过@get()的装饰就附带了URL信息。
#把函数装饰成带url信息的函数 给函数func加了__method__，__method__属性
